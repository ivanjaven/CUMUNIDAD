...
type EducationalAttainment = { id: number; name: string }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const educationalAttainments: EducationalAttainment[] = await Query({
      query:
        'SELECT educational_attainment_id AS id, educational_attainment_name AS name FROM educational_attainment',
      values: [],
    })

    // Format response
    const response: ResponseData & { educationalAttainments: EducationalAttainment[] } = {
      ...
      educationalAttainments: educationalAttainments.map((item) => ({
        id: item.id,
        name: item.name,
      })),
    }
